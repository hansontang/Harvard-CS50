...
db.execute(
    "CREATE TABLE IF NOT EXISTS orders (id INTEGER, user_id NUMERIC NOT NULL, symbol TEXT NOT NULL, \
...
def buy():
    ...
    if not result:
        return render_template(
            "buy.html", invalid=True, symbol=request.form.get("symbol")
        )
...
def sell():
    ...
    if owns[symbol] < shares:
        return render_template(
            "sell.html", invalid=True, symbol=symbol, owns=owns.keys()
        )
