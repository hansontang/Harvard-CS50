//Once I saw a project on Github which was hot at the time called c4, which claimed to implement a small C compiler with only 4 functions.
//What shocked me most about it was that it's a bootstrapped compiler which means it can compile itself. 
//And it did it with very little code to make a fairly functional C compiler.
//The things we need to do include building our own virtual machine as well as instruction sets, lexical parser, and syntax parser.
#include <stdio.h>
#include <stdlib.h>
#include <memory.h>
#include <string.h>
...
//PC  program counter, which holds a memory address that holds the next computer instruction to be executed.
//SP  pointer register always points to the current top of the stack.
//    Note that since the stack is at the higher address and grows toward the lower address, the value of SP decreases on entry.
//BP  Base address pointer,also used to point to some location on the stack, which is used when calling a function.
//AX  accumulate register, which in our virtual machine are used to hold the result of an instruction after its execution.
int * pc,           // pc register
    * sp,           // rsp register
    * bp;           // rbp register
...
int run_vm(int argc, char** argv) {
    ...
    *--sp = (int)tmp;
    ...
    while (1) {
        ...
        // native call
        // We already have implementations of these functions on our computers, 
		// so when compiling the compiler, the binary code for these functions is compiled into our compiler, 
		// and therefore these functions are available when running these instructions we provided on our compiler/virtual machine. 
		// In other words there is no need for us to implement them ourselves. :)
...
// after bootstrap use [int] instead of [int32_t]
